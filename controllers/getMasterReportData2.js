...
module.exports = getMasterReportData = async (req, res) => {
  try {
    ...
    if (loanIdsArray.length > 0) {
      ...
      receiptsData.forEach((receipt) => {
        ...
        if (emiChartMap[memberId]) {
          emiChartMap[memberId] = emiChartMap[memberId].map((emi) => {
            if (emi.emiStatus === "Pending" && status === "Paid") {
    ...
    loanDetails.map((loan) => {
      ...
      const baseLoanData = {
        ...
        emiDetails, // Add the calculated loan cycle
        agencyName:
