...
module.exports = getEucReportData = async (req, res) => {
  try {
    const { id } = req.query;
    ...
    const loanDetails = await member_details.findAll({
      ...
      having: sequelize.literal("COUNT(receiptsDetails.id) >= 2"),
      group: [
        "member_details.id",
        "businessDetails.id",
        "proposedLoanDetails.id",
        "bankDetails.id",
        "fk_member_details_belongsTo_center_centerId.id",
      ],
    });
