const { insurance_receipts, sequelize, member_details } = require("../models");

module.exports = addInsuranceReceipt = async (req, res) => {
  ...
  try {
    ...
    const status =
      receivedAmount >= pendingInsuranceAmount ? "paid" : "pending";

    // Create the receipt with specified fields
    await insurance_receipts.create(
      {
        memberId,
        managerId,
        disbursionDate,
