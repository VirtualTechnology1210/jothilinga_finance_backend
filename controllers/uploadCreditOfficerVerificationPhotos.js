...
const uploadCreditOfficerVerificationPhotos = async (req, res) => {
  try {
    ...
    uploadPhoto.single("verificationPhoto")(req, res, async (err) => {
      ...
      try {
        ...
        await credit_officer_verification_photos.create({
          memberId: member_id,
          roleId,
          photoName,
          fileName,
        ...
        res.status(201).json({
          message: "Credit Officer Verification Document uploaded successfully",
          memberId: member_id,
        });
      } catch (error) {
...
module.exports = uploadCreditOfficerVerificationPhotos;
