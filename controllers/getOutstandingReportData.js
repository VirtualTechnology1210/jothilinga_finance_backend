...
//         where: {
//           memberId: { [Op.in]: loanIdsArray },
//           // Include both "Paid" and "Pending" statuses (case-insensitive) to capture all actual collections
...
//           "collectedDate"
//         ],
//         order: [["collectedDate", "ASC"], ["createdAt", "ASC"]]
//       });
...
//         // Reset time on emiDueDate for a clean date comparison
//         const emiDateOnly = new Date(emiDueDate);
//         emiDateOnly.setHours(0, 0, 0, 0);

//         // An EMI is "genuinely overdue" if its due date has already PASSED as of today
//         const isAlreadyPassed = emiDateOnly < todayStart;
//         const isFullyPaid = emiData.isFullyPaid;
//         const paidAmount = emiData.paidAmount || 0;
//         const requiredAmount = emiData.emiAmount;

//         if (isAlreadyPassed && !isFullyPaid) {
//           // It is due in the past, and we haven't paid it fully
...
//           if (!firstOverdueDate) {
//             firstOverdueDate = emiDateOnly;
//             // DPD = Today - DueDate
//             dpd = Math.floor((todayStart - firstOverdueDate) / (1000 * 60 * 60 * 24));
//           }
//         }
//       }
...
//         lastPaidDate: combinedData.reduce((sum, loan) => sum + (loan.lastPaidDate || 0), 0),
...
//           bucket_1_30: combinedData.filter(loan => loan.bucket1 === "1-30").length,
...
//           bucket_90_plus: combinedData.filter(loan => loan.bucket1 === "90+").length
//         },
...
module.exports = getOutstandingReportData = async (req, res) => {
  try {
    ...
    uniqueLoanDetails.map((loan) => {
      ...
      // Calculate last paid date (MOVED OUTSIDE fromDate/toDate condition)
      let lastPaidDate = null;
      if (paidReceipts.length > 0) {
        const sortedReceipts = paidReceipts.sort((a, b) => new Date(b.paymentDate) - new Date(a.paymentDate));
        lastPaidDate = sortedReceipts[0].paymentDate;
      }
      ...
      const baseLoanData = {
        ...
        // ADD lastPaidDate to baseLoanData
        lastPaidDate: lastPaidDate ? new Date(lastPaidDate).toLocaleDateString("en-GB") : null,
      ...
      if (fromDate && toDate) {
        ...
        summaryData = {
          // REMOVED lastPaidDate from here since it's now in baseLoanData
          emiDatesInRange: emiDatesInRange.length
        };
      }
    ...
    res.status(200).json({
      ...
      summary: {
        ...
        // FIXED: Find the most recent lastPaidDate instead of summing
        mostRecentPaymentDate: (() => {
          const validDates = combinedData
            .map(loan => loan.lastPaidDate)
            .filter(date => date !== null);

          if (validDates.length === 0) return null;

          return validDates.sort((a, b) => {
            const dateA = a.split('/').reverse().join('-'); // Convert DD/MM/YYYY to YYYY-MM-DD
