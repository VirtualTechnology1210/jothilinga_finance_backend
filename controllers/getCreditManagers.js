...
module.exports = getCreditManagers = async (req, res) => {
  try {
    ...
    // Fetch all credit  managers
    const branchManagers = await manager_credentials.findAll({
    ...
    // Format the response
    const formattedResponse = branchManagers.map((manager) => {
    ...
    res.status(200).json({
      branchManager: formattedResponse,
    });
  } catch (error) {
