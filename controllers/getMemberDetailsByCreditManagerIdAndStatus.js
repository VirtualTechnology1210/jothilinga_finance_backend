...
module.exports = getAllMemberDetailsByCreditManagerIdAndStatus = async (
  req,
  res
) => {
  const creditManagerId = req.query.creditManagerId;
  const creditManagerStatus = req.query.creditManagerStatus;
  ...
  if (!creditManagerStatus) {
    return res.status(400).json({
      error: "creditManagerStatus query parameter is required.",
    });
  }
  ...
  try {
    ...
    const whereClause = { creditManagerStatus };
